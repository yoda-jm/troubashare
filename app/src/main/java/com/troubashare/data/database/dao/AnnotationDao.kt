...
interface AnnotationDao {
    ...
    // All annotations visible to a member (PERSONAL + shared ALL + PART for their parts)
    @Query("""
        ...
        WHERE fileId = :fileId
          AND (
            (memberId = :memberId)
            OR (scope = 'ALL')
            OR (scope = 'PART' AND partId IN (:partIds))
          )
    """)
    fun getVisibleAnnotations(fileId: String, memberId: String, partIds: List<String>): Flow<List<AnnotationEntity>>
    ...
    """)
    suspend fun getVisibleAnnotationsOnce(fileId: String, memberId: String, partIds: List<String>): List<AnnotationEntity>
